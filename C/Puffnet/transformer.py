...
class Transformer(nn.Module):
    ...
    def forward(self, style, mask, content, pos_embed_c, pos_embed_s):
        ...
        if pos_embed_s is not None:
            pos_embed_s = pos_embed_s.flatten(2).permute(0, 2, 1)

        content = content.flatten(2).permute(0, 2, 1)
        if pos_embed_c is not None:
            pos_embed_c = pos_embed_c.flatten(2).permute(0, 2, 1)
